...
const KEY_STEPS: Record<string, number> = {
  ArrowDown: 140, ArrowUp: -140, PageDown: 700, PageUp: -700, " ": 700,
  End: Number.MAX_SAFE_INTEGER, Home: Number.MIN_SAFE_INTEGER,
};
...
export default function AirlockHero({
  ...
  useEffect(() => {
    ...
    const onSeeked = () => {
      seeking = false;
      if (queued !== null) {
        const t = queued; queued = null; seeking = true;
        video!.currentTime = t;
      }
    };
    ...
    function paint(p: number) {
      ...
      if (videoRef.current) videoRef.current.style.transform = `scale(${1 + videoP * 0.06})`;
    ...
    function engageLock() {
      if (locked) return;
      locked = true; released = false; lockedY = window.scrollY;
    ...
    releaseRef.current = () => { target = shown = 1; moved = true; paint(1); releaseLock(); };
    ...
    const onWheel = (e: WheelEvent) => { if (consume(e.deltaY)) e.preventDefault(); };
    const onTouchStart = (e: TouchEvent) => { touchY = e.touches[0]?.clientY ?? 0; };
    const onTouchMove = (e: TouchEvent) => {
      const y = e.touches[0]?.clientY ?? touchY;
      const deltaY = touchY - y; touchY = y;
    ...
    const onScroll = () => {
      ...
      const y = window.scrollY; const climbing = y < lastY; lastY = y;
    ...
    if (!reduceMotion) {
      ...
      const frame = () => { shown += (target - shown) * 0.18; paint(shown); rafId = requestAnimationFrame(frame); };
  ...
  return (
    ...
    >
      <video
        ...
        style={{ opacity: ready ? 1 : 0, transformOrigin: "center center", willChange: "transform", transition: "opacity 0.6s ease" }}
      />
      ...
      <div ref={titleRef} className="pointer-events-none absolute inset-0 flex items-center justify-center px-[6%] text-center">
        <h1
          ...
          style={{ fontFamily: SANS, fontSize: "clamp(30px, 7vw, 96px)", color: palette.text, textShadow: "0 4px 30px rgba(0,0,0,0.55)", willChange: "transform, filter, opacity" }}
        >
          {title}
        </h1>
      </div>
      ...
      {tagline ? (
        <div ref={taglineRef} className="pointer-events-none absolute inset-0 flex items-center justify-center px-[8%] text-center opacity-0">
          <p
            ...
            style={{ fontFamily: SANS, fontSize: "clamp(20px, 3.4vw, 40px)", lineHeight: 1.2, color: palette.text, textShadow: "0 4px 24px rgba(0,0,0,0.6)" }}
          >
            {tagline}
          </p>
        </div>
      ...
      <div
        ...
        style={{ color: palette.muted, fontFamily: SANS, fontSize: "clamp(10px, 1.4vw, 12px)", fontWeight: 600, letterSpacing: "0.3em" }}
      >
        <span>{scrollHint}</span>
        <svg width="14" height="18" viewBox="0 0 14 18" aria-hidden="true" style={{ animation: "airlock-bounce 1.6s ease-in-out infinite" }}>
          <style>{`
            @keyframes airlock-bounce { 0%, 100% { transform: translateY(0); opacity: 0.5; } 50% { transform: translateY(5px); opacity: 1; } }
            @media (prefers-reduced-motion: reduce) { [style*="airlock-bounce"] { animation: none !important; } }
          `}</style>
      ...
      <div className="absolute inset-x-0 bottom-0 h-0.5" style={{ background: "rgba(255,255,255,0.12)" }}>
        <div ref={barRef} className="h-full w-full origin-left" style={{ background: palette.bar, transform: "scaleX(0)" }} />
      </div>

      {signature ? (
        <span
          className="absolute bottom-[clamp(10px,2vw,18px)] right-[clamp(12px,2.5vw,24px)] z-[2] font-medium"
          style={{ fontFamily: SANS, fontSize: "clamp(11px, 1.4vw, 13px)", color: palette.muted }}
        >
          by{" "}
          <a href={signature.url} target="_blank" rel="noopener noreferrer" className="no-underline transition-colors hover:opacity-100" style={{ color: "inherit" }}>
            {signature.name}
          </a>
        </span>
      ) : null}
